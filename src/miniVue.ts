 ...
 * - DSL functions: Text, Button, VStack, HStack
...
export function ZStack(...children:(VNode|string)[]):VNode {
  ...
  export function Image(src:string, frame?:{x:number,y:number,width:number,height:number}):VNode {
    const styleParts = [];
    if(frame){ styleParts.push(`position:absolute;left:${frame.x}px;top:${frame.y}px;width:${frame.width}px;height:${frame.height}px;`); }
  ...
  export function Rectangle(fill:string, frame:{x:number,y:number,width:number,height:number}):VNode {
    const style = `position:absolute;left:${frame.x}px;top:${frame.y}px;width:${frame.width}px;height:${frame.height}px;background:${fill};`;
  ...
  export function Circle(fill:string, frame:{x:number,y:number,width:number,height:number}):VNode {
    ...
    const style = `position:absolute;left:${frame.x}px;top:${frame.y}px;width:${diameter}px;height:${diameter}px;background:${fill};border-radius:50%;`;
    return { tag:'div', props:{ style }, children:[] };
  }
...
function createElement(v:VNode|string):Node {
    ...
    if(v.props){
      for(const [k,val] of Object.entries(v.props)){
        if(k==='onclick'&&typeof val==='function') el.addEventListener('click',val);
